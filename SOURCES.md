...
主資料集 master:**83,301 筆 / 2009–2026 / 116 場 / 4 來源**(海外賽另計)。
...
| **cyclist.org.tw 臺灣自行車聯賽**<br>(子來源:`results_txt.asp` 聯賽頁) | TCL 個人計時賽 ITT + 公路繞圈賽各分組(桃園繞圈賽完整)。聯賽成績走 `results_list.asp?pno=13` → `results_txt.asp?pno=N` 落地頁 → 成績公告 PDF,主 `cyclist_crawl` 抓不到。雙版面解析(組別在 col1 或 col3),只收有「組別」的個人成績列 | landing 頁 → 成績公告 PDF | 2025 | `cyclist_league.py` | 438 |
...
- **臺灣自行車聯賽 TTT(團隊計時賽_race)**:cyclist.org.tw 聯賽 PDF 內,**團隊式版面**(無「組別」欄、名次只在計分選手列、雙時間欄 完成時間/取第4名時間)→ `cyclist_league.py` 刻意排除(會把隊名當選手),需專屬 TTT parser;另 _積分排名/累計總排名_ 為衍生排名(非完賽列)不收。✅ 個人計時賽 ITT + 公路繞圈各分組已收。
